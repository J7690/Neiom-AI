...
FROM facebook_posts 
WHERE full_picture IS NOT NULL;
...
SELECT 'PHASE 6 READINESS' as audit_type,
       ...
       CASE 
           WHEN (SELECT COUNT(*) FROM facebook_posts WHERE message IS NOT NULL AND message != '') > 50
           THEN '✅ READY'
           ELSE '❌ INSUFFICIENT'
       END as status
...
SELECT 'PHASE 6 READINESS' as audit_type,
       ...
       CASE 
           WHEN (SELECT COUNT(*) FROM facebook_posts WHERE full_picture IS NOT NULL) > 20
           THEN '✅ READY'
           ELSE '❌ INSUFFICIENT'
       END as status
