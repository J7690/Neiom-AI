-- Extension des tables pour Phase 3 : Excellence Opérationnelle
...
CREATE TABLE IF NOT EXISTS studio_campaign_optimization (
    ...
    optimization_frequency TEXT DEFAULT 'daily' CHECK (optimization_frequency IN ('hourly', 'daily', 'weekly')),
...
CREATE TABLE IF NOT EXISTS studio_roi_tracking (
    ...
    roi_percentage NUMERIC GENERATED ALWAYS AS (
        CASE 
            ...
            THEN ROUND((returns_amount - investment_amount) / investment_amount * 100, 2)
            ELSE 0
        END
    ) STORED,
    ...
    cost_per_conversion NUMERIC GENERATED ALWAYS AS (
        CASE 
            WHEN conversion_count > 0 
            THEN ROUND(investment_amount / conversion_count, 2)
            ELSE 0
        END
    ) STORED,
    created_at TIMESTAMPTZ DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS studio_budget_optimization (
    ...
    remaining_budget NUMERIC GENERATED ALWAYS AS (total_budget - spent_budget),
...
CREATE TABLE IF NOT EXISTS studio_optimization_alerts (
    ...
    status TEXT DEFAULT 'active' CHECK (status IN ('active', 'acknowledged', 'dismissed', 'executed', 'completed')),
...
CREATE INDEX IF NOT EXISTS studio_campaign_optimization_next_optimization_idx ON studio_campaign_optimization(next_optimization_at);
CREATE INDEX IF NOT EXISTS studio_roi_tracking_campaign_idx ON studio_roi_tracking(campaign_id);
CREATE INDEX IF NOT EXISTS studio_roi_tracking_roi_idx ON studio_roi_tracking(roi_percentage);
CREATE INDEX IF NOT EXISTS studio_collective_intelligence_type_idx ON studio_collective_intelligence(intelligence_type);
CREATE INDEX IF NOT EXISTS studio_collective_intelligence_status_idx ON studio_collective_intelligence(validation_status);
CREATE INDEX IF NOT EXISTS studio_budget_optimization_campaign_idx ON studio_budget_optimization(campaign_id);
CREATE INDEX IF NOT EXISTS studio_budget_optimization_reallocation_idx ON studio_budget_optimization(next_reallocation_at);
CREATE INDEX IF NOT EXISTS studio_advanced_predictions_type_idx ON studio_advanced_predictions(prediction_type);
CREATE INDEX IF NOT EXISTS studio_advanced_predictions_accuracy_idx ON studio_advanced_predictions(prediction_accuracy DESC);
CREATE INDEX IF NOT EXISTS studio_performance_cohorts_type_idx ON studio_performance_cohorts(cohort_type);
CREATE INDEX IF NOT EXISTS studio_performance_cohorts_delta_idx ON studio_performance_cohorts(performance_delta DESC);
...
CREATE INDEX IF NOT EXISTS studio_optimization_alerts_potential_idx ON studio_optimization_alerts(impact_potential DESC);
