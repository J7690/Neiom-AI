...
// Receives WhatsApp Business webhook events and stores them in contacts/conversations/messages.
...
serve(async (req: Request): Promise<Response> => {
  ...
  if (req.method === "GET") {
    try {
      ...
      const { data, error } = await supabase.rpc("verify_whatsapp_challenge", {
        mode,
        verify_token: token,
        challenge,
      });
      ...
      return new Response(String(data ?? ""), { status: 200, headers: corsHeaders });
    } catch (_) {
  ...
  try {
    ...
    console.error("Unexpected error in whatsapp-webhook", e);
