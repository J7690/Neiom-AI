-- Implémentation des RPC marketing décisionnelles (version corrigée)
...
CREATE OR REPLACE FUNCTION approve_marketing_recommendation(p_recommendation_id TEXT, p_approved_by TEXT)
RETURNS TABLE (
    success BOOLEAN,
    message TEXT,
...
CREATE OR REPLACE FUNCTION reject_marketing_recommendation(p_recommendation_id TEXT, p_reason TEXT)
RETURNS TABLE (
    success BOOLEAN,
    message TEXT
) LANGUAGE PLPGSQL SECURITY DEFINER AS $$
...
CREATE OR REPLACE FUNCTION create_marketing_alert(p_alert_type TEXT, p_message TEXT, p_priority TEXT)
RETURNS TABLE (
    success BOOLEAN,
    alert_id TEXT,
    message TEXT
